 ...
 * @error: A #GError or %NULL
 *
 * Gets the distro name of the prepared system upgrade in the prepared transaction.
 *
 * Return value: the distro name, or %NULL if unset, free with g_free()
 *
 * Since: 1.1.1
 **/
gchar *
pk_offline_get_prepared_upgrade_name (GError **error)
{
	...
	return g_key_file_get_string (keyfile, "update", "name", error);
}
